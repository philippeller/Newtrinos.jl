...
function get_assets(physics, datadir = @__DIR__)
    ...
    AVG_MATTER_DENSITY_G_CM3 = 2.45    
    ELECTRON_FRACTION_YE = 0.5        
    NEUTRON_FRACTION_YN = 1.0 - ELECTRON_FRACTION_YE
    AVG_MOLAR_MASS_G_PER_MOL = 1.0
    ELECTRON_DENSITY = AVG_MATTER_DENSITY_G_CM3 * ELECTRON_FRACTION_YE / AVG_MOLAR_MASS_G_PER_MOL
    NEUTRON_DENSITY  = AVG_MATTER_DENSITY_G_CM3 * NEUTRON_FRACTION_YN / AVG_MOLAR_MASS_G_PER_MOL
    ...
    df_observed = CSV.read(joinpath(datadir,"sm_asimov.csv"), DataFrame; header=true, types=Dict("N" => Float64, "Ec" => Float64))
    observed = convert(Vector{Float64}, df_observed.N)    
    ...
    shape_unc_interp = LinearInterpolation(
         E_shape, Δshape;
    ...
    IBD_KINEMATIC_OFFSET = 0.782  # E_vis = E_nu - (m_n - m_p - m_e) ≈ 0.782 MeV

    E_arr_deposited_for_response = E_arr_neutrino_raw .- 0.782
    ...
    assets = (;
        ...
        background_interp,
        reactors,
        AVG_MATTER_DENSITY_G_CM3,
        ELECTRON_FRACTION_YE,
        NEUTRON_FRACTION_YN,
        AVG_MOLAR_MASS_G_PER_MOL,
        ELECTRON_DENSITY,
        NEUTRON_DENSITY,
...
function smear_average(E_arr_smear_local, smear_arr, E_bins, Delta_E_local)
    E_i_bins = E_bins .- Delta_E_local / 2
    E_f_bins = E_bins .+ Delta_E_local / 2
    smear_average_out = zeros(eltype(smear_arr), length(E_bins))
    for i in 1:length(E_bins)
        idx = (E_arr_smear_local .> E_i_bins[i]) .& (E_arr_smear_local .<= E_f_bins[i]) # find indices in E_arr_smear_local that fall into the current bin
        selected_values = smear_arr[idx]
        smear_average_out[i] = length(selected_values) > 0 ? mean(selected_values) : zero(eltype(smear_arr))
    end
    return smear_average_out
end

function osc_calc(params, physics, assets)
...
function get_expected(params, physics, assets)
    ...
    smeared_signal_spectrum = smear(E_vis_corr, spectrum_with_reactor_shape_sys, sigma_res_val, width=200) # width = 15 befofe
    ...
    modified_geo_counts = geo_counts_scaled_rate .* (1.0 .+ params.geo_shape_eps .* assets.GEO_SHAPE_UNC_FRACTION) 

    final_geo_counts = max.(modified_geo_counts, 0.0)

    other_background_counts = zeros(length(E_vis_corr))

    return final_signal_counts .+ final_geo_counts .+ other_background_counts
...
function get_plot(physics, assets)
    ...
    function plot_spectra(fitted_params,
                           true_params_for_data_gen, 
                           current_experiment_physics,
                           current_experiment_assets;
                           title_suffix::String="")
    
    model_events_fit = get_expected(fitted_params, current_experiment_physics, current_experiment_assets)

    model_events_true = get_expected(true_params_for_data_gen, current_experiment_physics, current_experiment_assets)

    E_vis_bins = current_experiment_assets.E_bins_visible 
        
    fig = Figure(size = (900, 700))
    ax_spec = Axis(fig[1, 1], 
                   xlabel=L"E_{vis} \text{ (MeV)}", 
                   ylabel="Events / Bin",
                   title="JUNO Spectrum Comparison" * title_suffix)

    CairoMakie.scatter!(ax_spec, E_vis_bins, current_experiment_assets.observed, 
                        label="Observed Data", color=:black, markersize=6)

    ys_err = sqrt.(max.(current_experiment_assets.observed, 0))
    ys_err[current_experiment_assets.observed .== 0] .= 1.0

    CairoMakie.errorbars!(ax_spec, E_vis_bins, current_experiment_assets.observed, ys_err, 
                          color=:black, whiskerwidth=0)

    CairoMakie.lines!(ax_spec, E_vis_bins, model_events_true, 
                      label="Model (Data Gen. Params)", color=:blue, linestyle=:dash, linewidth=2)

    CairoMakie.lines!(ax_spec, E_vis_bins, model_events_fit, 
                      label="Model (Fitted Params)", color=:red, linewidth=2)

    axislegend(ax_spec, position=:rt)

    ax_res = Axis(fig[2, 1], 
                  xlabel=L"E_{vis} \text{ (MeV)}", 
                  ylabel=L"\frac{\text{Data} - \text{Fit}}{\sigma_{\text{Data}}}",
                  title="Residuals" * title_suffix)
    
    residuals = (current_experiment_assets.observed .- model_events_fit) ./ ys_err
    residuals[.!isfinite.(residuals)] .= 0 
    
    CairoMakie.barplot!(ax_res, E_vis_bins, residuals, color=:gray)
    CairoMakie.hlines!(ax_res, [0], color=:black, linestyle=:dash)
    ylims!(ax_res, -5, 5) 
