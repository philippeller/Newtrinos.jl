...
function get_assets(physics; datadir = @__DIR__)
    ...
    assets = (
        ...
        numu_e_scale =0.9,#1.05,
        numu_e_bias = 0.1,
        nue_e_scale = 0.65,
        nue_e_bias = 0.5, #0.5,#0.02,
        percent=0.1,
        observed = observed
    )
...
function smeargauss(energy, probabilities, percent=0.04, width=10, e_scale=1.0, e_bias=0.0)
    """
    Apply energy resolution smearing to oscillation probabilities.
    Convolves probabilities with a Gaussian function to model detector resolution.
    """
    n = length(energy)
    out = zeros(n)
    ...
    for i in 1:n
        ...
        e=energy[i]*e_scale + e_bias
        norm = 0.0

        for j in max(1, i - width):min(n, i + width)
            sigma= percent * energy[j]
            coeff = exp(-0.5 * (e - energy[j])^2 / sigma^2)/(sqrt(2*pi)*sigma)
            norm += coeff
            out[i] += coeff * probabilities[j]
        end
        out[i] /= norm
...
function make_numu_predictions(params, physics, assets)
    ...
    energy_grid = logrange((assets.numu_data.energy_edges[2]/2), (5.0), length=100)   #why exp ???
...
function make_nue_predictions(params, physics, assets)
    ...
    p_nue_appearance./=sum(p_nue_appearance) # Normalize to total probability  #why??
    ...
    # Apply smearing
    p_nue_smeared = smearnorm(energy_grid, p_nue_appearance, 0.3, 5, assets.nue_e_scale, assets.nue_e_bias)
    #smeargauss(energy_grid, p_nue_appearance, 0.3, 50, assets.nue_e_scale, assets.nue_e_bias)

    p_nuebar_smeared = smearnorm(energy_grid, p_nuebar_appearance, 0.3, 5, assets.nue_e_scale, assets.nue_e_bias)
    #smeargauss(energy_grid, p_nuebar_appearance, 0.3, 50,assets.nue_e_scale, assets.nue_e_bias)
...
function get_plot(physics, assets)
    function plot(params)
        ...
        for i in 1:4
            ...
            println("Observed data for νμ Quartile $(i): sum=$(sum(observed))")
            println("Predicted data for νμ Quartile $(i): sum=$(sum(predicted))")
            println("MC histogram for νμ Quartile $(i): sum=$(sum(mc_histogram))")
        ...
        println( "Observed total νμ: sum=$(sum(observed_total_numu))")
        println( "Predicted total νμ: sum=$(sum(predicted_total_numu))")
        ...
        println( "MC total νμ: sum=$(sum(mc_total_numu))")
        ...
        println("Observed total ν̄μ: sum=$(sum(observed_total_numubar))")
        println("Predicted total ν̄μ: sum=$(sum(predicted_total_numubar))")
        println("MC total ν̄μ: sum=$(sum(mc_total_numubar))")
