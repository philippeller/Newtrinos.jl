...
function get_assets(physics, datadir = @__DIR__)
    ...
    # Import Data
    brnPE = CSV.read(joinpath(datadir, "csi/brnPE.txt")) # columns: PE, BRN PDF
    ninPE = CSV.read(joinpath(datadir, "csi/ninPE.txt")) # columns: PE, NIN PDF
    ssBkg = CSV.read(joinpath(datadir, "csi/dataBeamOnAC.txt")) # columns: PE, SS Background
    observed = CSV.read(joinpath(datadir, "csi/dataBeamOnC.txt")) # columns: PE, Observed Counts

    # Reconstruct bin edges from centers
    er_centers = midpoints(er_edges)

    pe_width = 5.0
    out_edges = LinRange(2.5, 62.5, pe_width)  # Bin edges: 5–60 PE, 5 PE width
...
function response_matrix_per_er_bin(keVnr, params, assets)
    ...
    if sum(gamma_weights) == 0
        return 0. # skip if no contribution
    end
    ...
    eff_vals = eff(out_centers, params)
...
function construct_response_matrix(params, assets)
    ...
    A = stack([response_matrix_per_er_bin(keVnr, params, assets) for keVnr in er_centers])
end
...
function build_rate_matrix(er_centers, enu_centers, nupar, physics, params):
    ...
    """
    stack([stack([physics.diff_xsec_csi(er, enu, params, nupar) for er in er_centers]) for enu in enu_centers])
end
...
function get_expected(params, physics, assets)
    ...
    flux = physics.flux(assets.exposure, assets.distance)
    ...
    for nupar in assets.isotopes:    
        ...
        rate_matrix = build_rate_matrix(
            ...
            flux[:, 1],         # Neutrino energy centers
            nupar,
            physics,
            params,
        )
    ...
    predicted_counts = int_rate * response_matrix'  # (n_samples, n_output_bins)
