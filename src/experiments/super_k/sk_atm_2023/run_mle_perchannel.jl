#!/usr/bin/env julia
# MLE with per-channel migration factors + fixed regularization
...
# Start from fixed-reg best-fit
@reset params.Δm²₃₁ = 0.002471892652340831
@reset params.θ₂₃ = asin(sqrt(0.4729))
@reset params.θ₁₃ = 0.14627305077324582
@reset params.δCP = 3.7699111843077517
@reset params.sk_total_norm = 1.0747
@reset params.xsec_cc1p1h_shape = 0.5354
@reset params.xsec_cc1pi_shape = 1.4237
@reset params.sk_flux_norm_high = 0.5208
@reset params.sk_flux_norm_low = -0.7978
@reset params.xsec_cc1p1h_subgev_norm = 0.8798531573440069
@reset params.xsec_cc1p1h_multigev_norm = 0.8798531573440069
@reset params.xsec_nc_norm = 1.1029
...
println("MLE with per-channel migration factors + fixed regularization")
println(@sprintf "Start: sin²θ₂₃=%.4f" sin(params.θ₂₃)^2)
flush(stdout)

llh, log_post, opt_params = Newtrinos.find_mle(likelihood, prior, params)

outpath = joinpath(@__DIR__, "mle_result_perchannel.jld2")
