...
function get_sys_flux(cfg::Barr)
    function sys_flux(flux, params)
        ...
        # Energy-dependent normalization tilt (smooth around 1 GeV)
        f_norm_tilt = e .^ (0.15 * (params.atm_flux_norm_ratio - 1))
        ...
        flux_nue3 = flux_nue2 .* f_spectral_shift .* f_uphorizontal .* f_updown .* f_norm_tilt .* f_k_pi_e .* params.atm_flux_norm
        flux_nuebar3 = flux_nuebar2 .* f_spectral_shift .* f_uphorizontal .* f_updown .* f_norm_tilt .* f_k_pi_e .* params.atm_flux_norm

        #numu
        uncert = (-0.16*log10e.^5 .+ 0.45*log10e.^4 .+ 0.48*log10e.^3 .+ 0.17*log10e.^2 .- 1.88*log10e .+ 1.88) / 100.
        f_uphorizontal = uphorizontal.(cz, 1 .+ uncert * params.atm_flux_uphorizonzal_sigma)
        flux_numu3 = flux_numu2 .* f_spectral_shift .* f_uphorizontal .* f_updown .* f_norm_tilt .* f_k_pi_mu .* params.atm_flux_norm
        flux_numubar3 = flux_numubar2 .* f_spectral_shift .* f_uphorizontal .* f_updown .* f_norm_tilt .* f_k_pi_mu .* params.atm_flux_norm
